...
def logincode():
    ...
    if res is None:
        ...
        session['lid'] = res[0]
        return '''<script>alert("Admin login success ");window.location="/userhome"</script>'''
...
def viewrequestandverifycardetails():
    id=session['lid']
    qry="SELECT `user`.`fname`,`lname`,`car_details`.* ,`scraprequest`.`date`,`scraprequest`.`id` FROM `user` JOIN`car_details` ON `car_details`.`lid`=`user`.`lid` JOIN `scraprequest` ON `scraprequest`.`carid`=`car_details`.`id` WHERE `scraprequest`.`lid`=%s"
    res=selectall2(qry,id)
...
def priceinfo_user1():
    ...
    return render_template('user/price1.html', val=msg,v=res1)
